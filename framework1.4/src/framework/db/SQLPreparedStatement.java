...
public class SQLPreparedStatement extends DBStatement {
		...
		try {
			...
			if (getParamSize() > 0) {
				for (int i = 1; i <= getParamSize(); i++) {
					if (getObject(i - 1) == null || "".equals(getObject(i - 1))) {
						pstmt.setNull(i, java.sql.Types.VARCHAR);
					} else if (getObject(i - 1) instanceof byte[]) {
						int size = ((byte[]) getObject(i - 1)).length;
						if (size > 0) {
							InputStream is = new ByteArrayInputStream((byte[]) getObject(i - 1));
	...
	public void set(int idx, double value) {
		set(idx, new Double(value));
	}

	public void set(int idx, int value) {
		set(idx, new Integer(value));
	}

	public void set(int idx, long value) {
		set(idx, new Long(value));
	}
