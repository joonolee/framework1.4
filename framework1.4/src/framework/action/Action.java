...
public abstract class Action {
	private Map _connMgrMap = new HashMap();
	 ...
	 * 
	 * @exception java.io.IOException Action에서 업무로직을 처리하는 동안 IO와 관련된 오류가 발생할 경우 
	 * @exception javax.servlet.ServletException 서블릿과 관련된 오류가 발생할 경우
	 */
	public void execute(HttpServlet servlet, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		setServlet(servlet);
		setRequest(request);
		setResponse(response);
		try {
			Method method = getMethod(getInput().getString("action"));
	...
	protected ConnectionManager getConnectionManager(String serviceName) {
		...
		return (ConnectionManager) this._connMgrMap.get(serviceName);
	}
	...
	private void destroy() {
		...
		Iterator iter = this._connMgrMap.keySet().iterator();
		while (iter.hasNext()) {
			connMgr = (ConnectionManager) this._connMgrMap.get(iter.next());
			if (connMgr != null) {
				connMgr.release();
				connMgr = null;
			}
