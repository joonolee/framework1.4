...
/**
	Table : <xsl:value-of select="@name"/> Table의 crud 처리
...
public class <xsl:value-of select="@class"/>DAO extends AbstractDao {
	// Update Only Map
	private static Map updateOnlyMap = new HashMap();
	...
	public String getUpdateSql(){
		<xsl:variable name="cnt" select="count(columns/column[not(@update ='none') and @update])"></xsl:variable>
		StringBuffer query = new StringBuffer();
