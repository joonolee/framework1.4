...
public class CreateMSSqlXMLFile {
	...
	private static final String _jdbcUrl = "jdbc:sqlserver://cims.homelinux.com:1433;databaseName=성도관리";
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if (_tableNameList != null && _tableNameList.length > 0) {
				for (int i = 0; i < _tableNameList.length; i++) {
					String tableName = _tableNameList[i];
	...
	private static void write(ResultSetMetaData meta, String name, Connection conn2) throws Throwable {
		...
		for (int c = 1; c <= meta.getColumnCount(); c++) {
			...
			if (meta.getColumnName(c).equals("UPDATEDATE")) {
				str.append(" insert=\"GETDATE()\" update=\"GETDATE()\"");
			}
